...
for n in range(len(bgYs) - 1, -1, -1):
    bg = pygame.image.load(
        os.path.join("assets", f"pixel_parallax_{n}.png")
    ).convert_alpha()
    scale = 0.3
...
for n in range(3, 0, -1):
    bg = pygame.image.load(
        os.path.join("assets", f"city_parallax_{n}.png")
    ).convert_alpha()
    scale = 1
    bg = pygame.transform.scale(bg, (scale * bg.get_width(), scale * bg.get_height()))
    bg_copies = ceil(W / bg.get_width())
...
airport_image_og = pygame.image.load(
    os.path.join("assets", f"airport.png")
).convert_alpha()
...
class Plane(object):
    ...
    def drag_coefficient(self):
        ...
        Cdrag_min = 0.012 * (
            7.867
            - 0.377 * self.flap_deflection
            + 0.046 * self.flap_deflection**2
            - 6.88e-04 * self.flap_deflection**3
        )
        ...
        if match_speed < self.critic_match:
            ...
            return Cdrag * 15 * (match_speed - self.critic_match) + Cdrag / sqrt(
                1 - (self.critic_match**2)
            )
    ...
    def lift_coefficient(self):
        ...
        Clift_max = (
            ...
            * (
                3.702
                + 0.159 * self.flap_deflection
                - 3.17e-3 * self.flap_deflection**2
                + (2.15e-05) * self.flap_deflection**3
            )
            - 0.2
        )
    ...
    def drag(self):
        ...
        drag_surface = self.front_surface * cos(
            angle_of_attack
        ) + self.wings_surface * sin(angle_of_attack)
        ...
        return (
            ...
            * drag_surface
            * self.collinear_speed() ** 2
            * spoilers_drag_factor
        )
    ...
    def lift(self):
        ...
        # Compute surface area experiencing drag
        lift_surface = self.front_surface * sin(
            angle_of_attack
        ) + self.wings_surface * cos(angle_of_attack)
    ...
    def horizontal_force(self):
        ...
        return (
            cos(pitch) * self.thrust()
            - cos(slope) * self.drag()
            - sin(pitch) * self.lift()
            - self.friction_wheels()
        )
    ...
    def update(self, screen):
        ...
        if self.brakes and self.altitude == 0 and self.horizontal_speed > 0:
            horizontal_acceleration = (
                horizontal_acceleration - self.braking_deceleration
            )
        ...
        pivot_wheels_pos = (
            self.x + plane_size[0] - plane_size[0] * 2.8 / 5,
            self.y + plane_size[1] - plane_size[1] / 5,
        )
        ...
        self.rect = original_sprite.get_rect(
            center=(self.x + plane_size[0] / 2, self.y + plane_size[1] / 2)
        )

        vector_center_to_pivot = (
            pygame.math.Vector2(pivot_wheels_pos) - image_rect.center
        )
        rotated_offset = vector_center_to_pivot.rotate(-self.pitch)
        rotated_image_center = (
            pivot_wheels_pos[0] - rotated_offset.x,
            pivot_wheels_pos[1] - rotated_offset.y,
        )
...
def draw_surface(x, y, length, height, color, transparent=False):
    ...
    if (
        x > position_range_screen[1]
        or (x + length) < position_range_screen[0]
        or y > altitude_range_screen[1]
        or (y + height) < altitude_range_screen[0]
    ):
        return
...
def draw_sprite(sprite, x, y):
    ...
    if (
        x > position_range_screen[1]
        or (x + sprite.get_width()) < position_range_screen[0]
        or y > altitude_range_screen[1]
        or (y + sprite.get_height()) < altitude_range_screen[0]
    ):
        return
    ...
    posX = (x - position_range_screen[0]) / pixel_to_length  # pixels
    posY = (
        H - (y - altitude_range_screen[0]) / pixel_to_height - sprite.get_height()
    )  # pixels
...
def landed_on_airport(plane):
    ...
    for n, airport_start in enumerate(AIRPORTS):
        if abs(plane.position) >= airport_start and abs(plane.position) <= (
            airport_start + RUNWAY_LENGTH
        ):
            return n + 1
    return False
...
def draw_background(sprite, parallax_X, altitude):
    ...
    posY = (
        H
        - (altitude - vertical_scroll_factor * altitude_range_screen[0])
        / pixel_to_height
        - sprite.get_height()
    )  # pixels
...
def updateScreen():
    ...
    for n, airport_position in enumerate(AIRPORTS):
        ...
        if n > 0:
            draw_sprite(
                airport_image,
                x=airport_position + RUNWAY_LENGTH - 300,
                y=RUNWAY_HEIGHT * 3 / 4,
            )
        else:
            draw_sprite(
                airport_image, x=airport_position + 100, y=RUNWAY_HEIGHT * 3 / 4
            )
        # Draw the runway
        draw_surface(
            x=airport_position - CITY_EXTENSION,
            y=RUNWAY_HEIGHT * 3 / 4,
            length=RUNWAY_LENGTH + 20 + 2 * CITY_EXTENSION,
            height=2 * RUNWAY_HEIGHT,
            color=(210, 210, 210),
        )
        draw_surface(
            x=airport_position,
            y=RUNWAY_HEIGHT * 3 / 4,
            length=RUNWAY_LENGTH + 20,
            height=2 * RUNWAY_HEIGHT,
            color=(194, 194, 194),
        )
        draw_surface(
        ...
        for n in range(5):
            draw_surface(
                x=airport_position + 2,
                y=RUNWAY_HEIGHT / 2 - 2 * n - 0.25,
                length=MARKINGS_LENGTH / 2,
                height=0.5,
                color=yellow,
            )
            draw_surface(
                x=airport_position + RUNWAY_LENGTH - MARKINGS_LENGTH,
                y=RUNWAY_HEIGHT / 2 - 2 * n - 0.25,
                length=MARKINGS_LENGTH / 2 - 2,
                height=0.5,
                color=yellow,
            )
    ...
    if not plane.crashed:
        shadow_start = (
            0.05 * plane.length
            + position_range_screen[0]
            + (plane.x / W) * (position_range_screen[1] - position_range_screen[0])
        )
    ...
    if plane.slope < 0:
        touchdown_prediction = (
            cos(plane.slope)
            * plane.collinear_speed()
            * (-plane.altitude / sin(plane.slope) / plane.collinear_speed())
        )
        touchdown_prediction = f"{touchdown_prediction:.0f}"
    else:
        touchdown_prediction = "-"
    nearest_runway_distance = min(
        [
            airport - plane.position
            for airport in AIRPORTS
            if (airport - plane.position) > 0
        ]
    )
    ...
    flight_indicators = []
    flight_indicators.append(
        largeFont.render(f"Thrust: {plane.thrust_level*100:.0f} %", 1, white)
    )
    flight_indicators.append(
        largeFont.render(f"Flaps: {plane.flap_deflection:.0f}°", 1, white)
    )
    flight_indicators.append(largeFont.render(f"Pitch: {plane.pitch:.1f}°", 1, white))
    flight_indicators.append(
        largeFont.render(f"H.Speed: {plane.horizontal_speed:.1f} m/s", 1, white)
    )
    flight_indicators.append(
        largeFont.render(f"V.Speed: {plane.vertical_speed:.1f} m/s", 1, white)
    )
    flight_indicators.append(
        largeFont.render(f"AOA: {plane.angle_of_attack:.1f}°", 1, white)
    )
    flight_indicators.append(
        largeFont.render(f"Altitude: {plane.altitude:.1f} m", 1, white)
    )
    flight_indicators.append(
        largeFont.render(f"Position: {plane.position:.1f} m", 1, white)
    )
    flight_indicators.append(
        largeFont.render(f"Fuel: {plane.mass_fuel:.0f} kg", 1, white)
    )
    ...
    flight_indicators.append(
        largeFont.render(f"Next runway: {nearest_runway_distance:.0f} m", 1, white)
    )
    ...
    for n, (objective, condition) in enumerate(zip(objectives, conditions)):
        ...
        pygame.draw.rect(
            screen, white, (W - panel_width - 80, 45 + n * 20, 15, 15), width=width
        )
...
def screen_configuration(W, H):
    ...
    plane.crash_sprite = pygame.transform.scale(
        crash_image_og, (2 * plane_size[0], plane_size[0])
    )
    airport_image = pygame.transform.scale(
        airport_image_og, [x / 9 for x in airport_image_og.get_size()]
    )
...
conditions.append(
    lambda plane: landed_on_airport(plane) > 1 and abs(plane.horizontal_speed) < 5
)
...
while run:
    ...
    if (round(frame / FPS, 1) % 1) == 0:
        ...
        for n in range(N_new_clouds):
            cloud = {
                "position": randint(
                    round(position_range_screen[1]),
                    round(1.5 * position_range_screen[1]),
                ),
                "altitude": randint(
                    round(0.5 * altitude_range_screen[1]),
                    round(altitude_range_screen[1]),
                ),
                "image": pygame.transform.scale(
                    cloud_image, [x * randint(1, 4) for x in cloud_image.get_size()]
                ),
                "speed": 1 / randint(1, 8),
            }
    ...
    clouds = [
        cloud
        for cloud in clouds
        if (cloud["position"] + cloud["image"].get_width()) > position_range_screen[0]
    ]
    ...
    close_to_airports = [
        plane.position > (airport - CITY_EXTENSION / 2)
        and plane.position
        < (airport + RUNWAY_LENGTH + CITY_EXTENSION / 2 - city_bgs[0].get_width())
        for airport in AIRPORTS
    ]
    arriving_to_airports = [
        position_range_screen[1] > (airport - CITY_EXTENSION - 100)
        and position_range_screen[1] < (airport - CITY_EXTENSION)
        for airport in AIRPORTS
    ]
    for n, (bg, bgXs, speed) in enumerate(
        zip(city_bgs, city_bgsX, city_parallax_speed)
    ):
        for m, bgX in enumerate(bgXs):
            bgX -= speed * positional_change / pixel_to_length
