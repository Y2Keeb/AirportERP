...
class AirlineScreen(BaseWindow):
    ...
    def __init__(self, root, username=None, view_manager=None):
        ...
        self.view_state = {"role": "airline"}
        self.tree = None
        self.create_menu_bar(["help", "about", "exit", "logout"])

        self.frame_main = ctk.CTkFrame(self.root)
        self.frame_main.place(
            relx=0.5, rely=0.52, anchor="center", relwidth=0.9, relheight=0.9
        )
        self.frame_form = ctk.CTkFrame(
            self.frame_main, border_color="black", border_width=3
        )
        self.frame_tree = ctk.CTkFrame(
            self.frame_main, border_color="black", border_width=3
        )
        ...
        lbl_pending_text = ctk.CTkLabel(
            self.frame_main, text="Current pending flights:", font=("Arial", 14, "bold")
        )
        lbl_pending_text.place(relx=0.53, rely=0.06)

        self.frame_form.place(
            relx=0.25, rely=0.50, anchor="center", relwidth=0.45, relheight=0.8
        )
        self.frame_tree.place(
            relx=0.75, rely=0.50, anchor="center", relwidth=0.45, relheight=0.8
        )
        ...
        self.lbl_departure_date = ctk.CTkLabel(
            self.frame_form, text="Departure date (YYYY-MM-DD): "
        )
        self.lbl_arrival_date = ctk.CTkLabel(
            self.frame_form, text="Arrival date (YYYY-MM-DD): "
        )
        ...
        self.btn_offer_flight = ctk.CTkButton(
            self.frame_main, text="Offer Flight", command=self.complete_flight_offer
        )
        self.btn_clear_form = ctk.CTkButton(
            self.frame_main, text="Clear", command=self.clear_form
        )
    ...
    def complete_flight_offer(self):
        ...
        try:
            ...
            if cursor.fetchone():
                messagebox.showinfo(
                    "Duplicate", "This flight already exists in pending flights."
                )
                cursor.close()
                return
