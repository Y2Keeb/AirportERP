...
class AdditionalPackageScreen(BaseWindow):
    """Screen for selecting additional packages for a flight booking."""
    ...
    def __init__(
        ...
        super().__init__(
            root,
            title=(
                f"Additional Packages - {username}"
                if username
                else "Additional Packages"
            ),
        )
        ...
        flight_id, airline, from_location, departure, to_location, price = (
        ...
        self.frame_content.place(
            relx=0.5, rely=0.52, anchor="center", relwidth=0.8, relheight=0.8
        )
        ...
        self.flight_info_label = ctk.CTkLabel(
            self.frame_content, text=flight_info, font=("Arial", 14, "bold")
        )
        ...
        self.checkbox_package1 = ctk.CTkCheckBox(
            self.frame_additions,
            text="Priority boarding – 15 €",
            variable=self.package1_var,
            command=self.update_checkbox_total,
        )
        self.lbl_package1 = ctk.CTkLabel(
            self.frame_additions, text="Board the plane first."
        )

        self.package2_var = ctk.BooleanVar()
        self.checkbox_package2 = ctk.CTkCheckBox(
            self.frame_additions,
            text="Unaccompanied minor service – 55 €",
            variable=self.package2_var,
            command=self.update_checkbox_total,
        )
        self.lbl_package2 = ctk.CTkLabel(
            self.frame_additions,
            text="We will supply a babysitter,while you enjoy a drink in first class!",
        )

        self.package3_var = ctk.BooleanVar()
        self.checkbox_package3 = ctk.CTkCheckBox(
            self.frame_additions,
            text="Pet in cabin + 10 €",
            variable=self.package3_var,
            command=self.update_checkbox_total,
        )
        self.lbl_package3 = ctk.CTkLabel(
            self.frame_additions,
            text="Pets fly in the cabin? Absolutely. Dogs, cats, emotional support hamsters,we’re here for it!\nIn fact, we will even GIVE you money to bring one!",
            justify="left",
        )

        self.buy_button = ctk.CTkButton(
            self.frame_content, text="Buy", command=self.finalize_purchase
        )

        self.lbl_flight_price_label = ctk.CTkLabel(
            self.frame_total_price, text="Flight: "
        )
        self.lbl_flight_price = ctk.CTkLabel(
            self.frame_total_price, text=f"{float(price):.2f} €"
        )
        self.lbl_additional_package_label = ctk.CTkLabel(
            self.frame_total_price, text="Selected packages:"
        )
        ...
        self.total_price_label = ctk.CTkLabel(
            self.frame_total_price, text=f"{total_price:.2f} €"
        )
    ...
    def create_widgets(self):
        ...
        # price frame
        self.lbl_flight_price_label.grid(
            row=2, column=0, padx=25, pady=(40, 10), sticky="w"
        )
        self.lbl_flight_price.grid(row=2, column=1, padx=25, pady=(40, 10))
        self.lbl_additional_package_label.grid(
            row=3, column=0, padx=25, pady=10, sticky="w"
        )
    ...
    def payment_completed(self, success):
        ...
        if success:
            ...
            self.view_manager.show_view(
                "UserScreen", user_id=self.user_id, username=self.username
            )
        else:
    ...
    def update_total_price(self, flight_price):
        ...
        try:
            ...
            if hasattr(self, "discount_applied") and self.discount_applied:
                ...
                self.lbl_discount_label.configure(
                    text=f"Discount ({getattr(self, 'discount_percent', 0)}%): -"
                )
            else:
    ...
    def logout(self):
        ...
        try:
            ...
            kiosk_login_screen = KioskLoginScreen(
