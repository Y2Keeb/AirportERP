...
class MyBookings:
    def __init__(self, root, user_id, previous_window=None):
        ...
        self.title_label = ctk.CTkLabel(
            self.frame_main, text="My Bookings", font=("Arial", 16, "bold")
        )
    ...
    def load_bookings(self):
        ...
        self.cursor.execute(
            "SELECT id, flight_id, booking_date, status FROM bookings WHERE user_id = %s",
            (self.user_id,),
        )
        ...
        if bookings:
            for booking in bookings:
                ...
                self.tree.insert(
                    "",
                    tk.END,
                    values=(booking[3], departure_info, destination_info, flight_info),
                )
        else:
            CTkMessagebox(
                title="No Booking",
    ...
    def get_flight_info(self, flight_id):
        ...
        self.cursor.execute(
            "SELECT airline, departure, arrival FROM flights WHERE id = %s",
            (flight_id,),
        )
